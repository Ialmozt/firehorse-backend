"""
PGMQ Worker for Firehorse - processes jobs from queue_orders using REST API.
"""
import asyncio
import logging
import time
...
from app.services.supabase_client_v2 import get_supabase_client
from app.services.pgmq_rest_client import get_pgmq_client
...
class PGMQWorkerREST:
    """Worker that polls PGMQ queue via REST API and processes jobs"""
    
    def __init__(self):
        self.supabase = get_supabase_client()
        self.deepseek = DeepSeekClient()
        self.pgmq = get_pgmq_client()
        self.running = False
    ...
    async def poll_job_queue(self) -> Optional[Dict[str, Any]]:
        """
        Poll job_queue for next available job via REST API.
        
        Returns:
            Job data if available, None otherwise
        """
        try:
            job = self.pgmq.read_job(
            ...
            if job:
                logger.info(f"📥 Received job via REST: {job.get('msg_id')}")
                return job
            
            return None
                
        except Exception as e:
            logger.error(f"❌ Error polling job queue via REST: {str(e)}")
            return None
    ...
    async def acknowledge_job(self, job_id: int) -> bool:
        ...
        try:
            success = self.pgmq.delete_job("job_queue", job_id)
            if success:
                logger.debug(f"✅ Acknowledged job {job_id} via REST")
            return success
        except Exception as e:
    ...
    async def reject_job(self, job_id: int) -> bool:
        ...
        try:
            # Для REST API используем архив вместо reject
            # В PGMQ через REST нет прямой функции reject, поэтому используем archive
            success = self.pgmq.archive_job("job_queue", job_id)
            if success:
                logger.debug(f"🔄 Rejected job {job_id} for retry via REST")
            return success
        except Exception as e:
    ...
    async def move_to_dlq(self, job_id: int, reason: str) -> bool:
        ...
        try:
            ...
            # Отправляем в DLQ
            dlq_msg_id = self.pgmq.send_job("dlq_job_queue", dlq_message)
            
            # Удаляем из основной очереди
            await self.acknowledge_job(job_id)
            
            if dlq_msg_id:
                logger.warning(f"⚠️ Moved job {job_id} to DLQ via REST: {reason}")
                return True
            else:
                logger.error(f"❌ Failed to send job {job_id} to DLQ")
                return False
    ...
    async def run(self):
        """Main worker loop"""
        logger.info("🚀 Starting PGMQ Worker (REST API version)...")
        
        # Test PGMQ connection
        if not self.pgmq.test_connection():
            logger.warning("⚠️ PGMQ REST connection test failed, but continuing...")
